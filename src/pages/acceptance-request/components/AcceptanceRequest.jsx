...
export default function AcceptanceRequest() {
  ...
  const acceptanceRequestList = useSelector((state) => state.acceptanceRequest.acceptanceRequestList);
  const loading = useSelector((state) => state.acceptanceRequest.loading);

  const editingAcceptanceRequest = useSelector((state) => state.acceptanceRequest.editingAcceptanceRequest);
        ...
        title={
          <>
            ...
            <Button
              type="primary"
              icon={<SearchOutlined />}
              onClick={() => dispatch(getAcceptanceRequestList(searchTerm))}
            >
              Tìm kiếm
            </Button>
          </>
        ...
        {!loading && (
          <TableContainer>
            <Table
              ...
              size="middle"
              rowClassName={(record) => (editingAcceptanceRequest?.id === record.id ? 'active-row' : '')}
            />
          </TableContainer>
        )}
      </Card>

      {openDetail && <ViewAcceptanceRequest open={openDetail} onClose={handleCloseDetail} />}
    </>
  );
}
